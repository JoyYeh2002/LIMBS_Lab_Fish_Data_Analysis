%% step01b_head_clean_data_GM_calculate.m
% Updated 04.27.2024
% LIMBS Lab
...
% all_fish = load(fullfile(abs_path, 'data_clean_head.mat'));
...
for fish_idx = 1 : numFish
    ...
    for il = 1 : num_il_levels
        ...
        all_fish(fish_idx).data(il).GMMean = {};
        all_fish(fish_idx).data(il).gmGainMean = {};
        all_fish(fish_idx).data(il).gmPhaseMean = {};

        all_fish(fish_idx).data(il).CP = {};
        all_fish(fish_idx).data(il).cpGain = {};
        all_fish(fish_idx).data(il).cpPhase = {};

        all_fish(fish_idx).data(il).CPMean = {};
        all_fish(fish_idx).data(il).cpGainMean = {};
        all_fish(fish_idx).data(il).cpPhaseMean = {};

        % Get the average in this il level
        data_mean = cell2mat(all_fish(fish_idx).data(il).fishXMean);
        FM = fftshift(fft(data_mean));
        GM = zeros(size(k));
        PM = zeros(size(k));
        ...
        all_fish(fish_idx).data(il).GMMean = GM;
        all_fish(fish_idx).data(il).gmGainMean = abs(GM);
        all_fish(fish_idx).data(il).gmPhaseMean = PM;

        CP = GM ./ (1 - GM);
        all_fish(fish_idx).data(il).CPMean = CP;

        CPM = rad2deg(unwrap(angle(CP)));
        % CPM = calibratePhase(CP, GM); % Don't calibrate CP phase
        all_fish(fish_idx).data(il).cpGainMean = abs(CP);
        all_fish(fish_idx).data(il).cpPhaseMean = CPM;

        %% Calculate for all the individual trials
...
save([abs_path, 'data_clean_head_with_GM.mat'], 'all_fish');
