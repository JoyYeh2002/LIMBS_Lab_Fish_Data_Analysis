...
%
% Content:
% - Create the base struct: "result_tail_positions.m" (save only the tail
% points)
% - Calculate tail point FFT position and velocity
% - Save to the "result_tail_fft.mat" struct
% - Plot the FFT pos, FFT velocity vs. illuminance plot for 5 fish in
% archive, one fish in "/figures" folder
%
% Output:
...
out_archive_path = fullfile(parent_dir, 'figures_archive\fig04b_tail_FFT\');
...
%% 1. Load the full body clean struct
...
% [Input] Select fish #1 and #3 to be in the final paper ("/figures"
% folder)
sample_fish_idx = [1, 3];
...
for i = 1 : 5
    ...
    %% ----------------- Ploltting FFT for 5 fish with colored legend, 5 figs total ------------------
...
%% 4. Save two structs: "raw" (with x- and y- position data) and "res" (with FFT calculations)
save([abs_path, 'result_tail_positions.mat'], 'raw');
disp("Tail point x- and y- position data saved in 'result_tail_positions.mat'.")
